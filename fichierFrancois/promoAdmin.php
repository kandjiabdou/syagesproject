	...
	if (isset($_POST["formulaire"])){
		if ($_POST["formulaire"]=="creerPromotion"){
			if (    !empty($_POST["nomPromo"]) and !empty($_POST["dateDebut"]) and !empty($_POST["dateFin"]) and 
				...
				$_POST["nomPromo"]=ucwords($_POST["nomPromo"]);//majuscule
	                ...
	                <div class="tababs marginauto">
	                    <table>
                    		...
                    		foreach ($daeu as $value) {
                    			...
                    			$matiere= $bd->all_matiereDAEU($value); // Tous les matieres d'un daeu (A ou B)

                    			$matieres["obligatoire"] = explode("; ",ucwords($bd->obl_matiereDAEU($value)[0]));

                    			foreach ($matieres["obligatoire"] as $keymat=> $mat) {
                                	if (!in_array($mat,$matiere)){
                                        unset($matieres["obligatoire"][$keymat]);
                                    }
                                }

                    			foreach ($matiere as $keyMatiere => $valueMatiere) { // On ajoute les matieres optionnelles
                                    if (!in_array($valueMatiere,$matieres["obligatoire"])){
                                        $matieres["optionnelle"][$keyMatiere]=$valueMatiere;
                                    }
                                }
                    ...
                    <table class="promotionEnCours">
                    	...
                    	<?php
                            ...
                            <td>'.$value["DateDebut"].'</td><td>'.$value["DateFin"].'</td><td>'.$bd->get_nbElevePromotion($value["idPromotion"])[0].'</td><td><a>voir absences</a></td></tr>';
                        		...
                        		$matiere = $bd->get_matiere($value["idPromotion"]); // On prend les matieres de la promotion

								// On prends les matieres obligatoires
                                $matieres["obligatoire"]=explode("; " ,ucwords($value["matieres"]," "));

                                //On verifie si les matieres obligatoires existent dans la base
								foreach ($matieres["obligatoire"] as $keymat=> $mat) {
                                	if (!in_array($mat,$matiere)){
                                        unset($matieres["obligatoire"][$keymat]);
                                    }
